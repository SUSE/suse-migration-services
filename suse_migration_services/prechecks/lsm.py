...
#
"""Call prechecks for LSM migration (selinux to apparmor)"""
import logging
import json
import os
import subprocess
...
def check_lsm(migration_system=False):
    ...
    if _apparmor_enabled():
        try:
            ...
            if _apparmor_primitive_check():
                # additional profiles added to /etc/apparmor.d/, asking to install 'aa-status' for proper checks
                exit(2)
            return
    ...
    if abort_migration:
        log.error('AppArmor changes detected that can not be mitigated, aborting migration.')
        exit(1)
    return
...
def _apparmor_extended_profiles_modified():
    ...
    for pkg in ADDITIONAL_AA_PROFILES:
        ...
        if __find_modified_profiles(pkg, path_filter):
            log.error("Modified apparmor profiles found, please verify changes to files under '{}': 'rpm -V {}'".format(path_filter, pkg))
            retval = True
    return retval
