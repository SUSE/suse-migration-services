    ...
    ** Managing /etc/resolv.conf during migration **
    ...
    def setup_target_root(self):
        ...
        if os.path.islink(conf):
            ...
            if not link_target:
                log.warning("Failed to resolv link {} with root:{}".format(conf, self.root_path))
            elif link_target.endswith('/netconfig/resolv.conf'):
                log.info('Delete link {} -> {}'.format(conf, link_target))
                os.unlink(conf)
            elif link_target.endswith('/NetworkManager/resolv.conf'):
                log.info('Delete link {} -> {}'.format(conf, link_target))
                os.unlink(conf)
            else:
                log.info('Keep link {} -> {}'.format(conf, link_target))
        else:
    ...
    def setup_live_system(self, dest_root='/'):
        ...
        if os.path.islink(src):
            ...
            if not link_target:
                log.warning("Failed to resolv link {} with root:{}".format(src, self.root_path))
            else:
    ...
    def read_netconfig_md5(self):
        md5_file = os.path.join(self.root_path, 'var/adm/netconfig/md5/etc/resolv.conf')
        if os.path.exists(md5_file):
            with open(md5_file, 'r') as f:
                match = re.search(r'^#([^\n\r]*)\r?\n([0-9a-fA-F]{32})', f.read(), re.MULTILINE)
                if match:
    ...
    def is_custom_resolv_conf(self, file):
        ...
        # netconfig checks via md5sum if the resolv.conf is customized.
