...
Enterprise Server 15. For a service pack upgrade, i.e. from one Service Pack
(SP) to another in SLE 15, or minor release version upgrade, for example from
.0 to .1 in SLE 16, the existing zypper migration workflow provides the
supported upgrade path. The distribution migration system provides the
ability to upgrade across major distributions without the need to use the
next major version installation media to perform the system upgrade.
...
SUSE Customer Center (SCC) or in th ePublic CLoud to the SUSE operated
update infrastructure. Administrative work may be required when
...
system to the latest available packages. Call the following commands to
perform this update:
+
[listing]
----
...
SLES 12 SP5 is the only supported migration starting point for the migration
to SLES 15.

SLES 15 SP7 is the only supported migration starting point for the migration
to SLES 16.
...
This script is run during the install of the SLES15-Migration or
SLES16-Migration package on a SLES 12 SP5 or SLES 15 SP7 system, respectively.
The pre-checks can also be run anytime using
...
== Installation
The distribution migration system is distributed from the Public Cloud module
in SLE 12 and therefore this module has to be enabled on the system
prior to upgrade. For running on-demand instances this module is already
enabled. In SLE 15 the package is part of the Basesystem module
that is automatically enabled on all registered systems.
...
+
This package contains the live migration image to upgrade to SLES_SAP15 SP4
with SAP extensions.
SLES16-Migration and SLES16-SAP_Migration::
These packages contain the live migration image to upgrade to the respective
16.0 distribution.
...
the SLES15-Migration and SLES16-Migration packages. The second method to
invoking the migration process is via reboot after installing the
suse-migration-sle15-activation or suse-migration-sle16-activation
package, respectively.

Option 1 - Trigger via run_migration, using a 12 to 15 migration on SLES
as an example::
+
[listing]
----
...
upgrade image installed by the SLES{$VERSION}-Migration package.
+
For a migration to SLE 16 on the s390x architecture the *-Migration package
is the only package available and the the `run_migration` method is the only
option to start the migration process.
+
...
suse-migration-sle15-activation or suse-migration-sle16-activation package,
respectively, covers the all use cases except in cases where there is no
direct access to the root file system from the bootloader. During
installation of the suse-migration-sle{15|16}-activation package the bootloader
configuration is modified  such that on the next boot the system will boot
into the upgrade image. This in turn starts the automated distribution
migration process.
...
Add Custom Zypper Call Options::
For the main zypper migration call and for eventual further package
installations after the migration, it is possible to add custom options
to the zypper call. The following example shows how to add the `--recommends`
option to the zypper migration call and the `--no-recommends` option to
the zypper package installation call. If there are further installation calls
in the process of the system migration depends on the subsystems that are
subject to the migration. For example, if the system is based on apparmor
and the migration is from SLE 15 to SLE 16 additional packages from the
`selinux` stack will be installed. Only in such cases the `install` section
of the configuration file is used to add custom options to that zypper call.
As such the `install` should not be confused with a general zypper
installation call and it may well happen that there are no further package
installations as part of the migration process.
+
[listing]
----
zypper:
...
+
[NOTE]
The provided option settings are add-on settings to the caller options
we use for the zypper migration/dup workflow and for eventual
...
setting. The product must be specified with
...
+
[WARNING]
Changing the default product may lead to unsupported territory and
...
When migrating from `wicked` to `NetworkManager`, this applies to a
migration from 15 to 16, the `wicked2nm` tool may issue warnings. By
default, these warnings abort the migration process.
...
In addition, the distribution migration RPM packages `SLES{15|16}-Migration`
and `suse-migration-sle{15|16}-activation` will be removed.
...
* For the migration from SUSE Linux Enterprise 12 to 15 the migration system
  is primarily intended for Public Cloud instance upgrade use. The
  ...
  For the migration from SUSE Linux Enterprise 15 to 16 the migration system
  is generally supported for all use cases.
...
* Upgrade is not supported for systems having the SLE 12 HPC module installed.
  ...
  there is no migration path from SLE 12 (with the HPC module) to SLE 15 HPC.
...
* Migration initiation for a cloud instance is only supported via a reboot.
  ...
  provided with the suse-migration-sle{15|16}-activation package. We recommend
...
* Public Cloud instances from SUSE images have a custom `/etc/motd` file
  ...
  updated manually after the upgrade to indicate the distribution upgrade.
