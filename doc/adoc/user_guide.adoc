...
* The distribution migration system will migrate the system mounted in `/` (main
  system), in the scenario where other systems are mounted in different paths,
  those systems will not be taken into account for the migration.
