...
from suse_migration_services.units.migrate import MigrateSystem, main
...
class TestMigration(object):
    ...
    def setup(self, mock_get_migration_config_file):
        ...
        mock_get_migration_config_file.return_value = '../data/migration-config-verbose.yml'
        self.migration_config_verbose = MigrationConfig()
        mock_get_migration_config_file.return_value = '../data/migration-config-zypper-dup.yml'
        self.migration_config_dup = MigrationConfig()
        mock_get_migration_config_file.return_value = '../data/migration-config-solver-case.yml'
        ...
        self.migration_config_zypper_args = MigrationConfig()
    ...
    def test_main_zypper_dup_raises(
        ...
        migration_config = Mock()
        migration_config.is_zypper_migration_plugin_requested.return_value = False
        mock_MigrationConfig.return_value = migration_config
        zypper_call = Mock()
        mock_Zypper_run.return_value = zypper_call
        mock_get_system_root_path.return_value = '../data'
    ...
    def test_main_zypper_migration_plugin(
        ...
        mock_open.assert_called_once_with('/var/log/distro_migration.exitcode', 'w')
    ...
    @patch('suse_migration_services.defaults.Defaults.get_system_root_path')
    @patch('suse_migration_services.units.migrate.MigrationConfig')
    def test_main_zypper_migration_plugin_with_zypper_args(
        self,
        mock_MigrationConfig,
        mock_get_system_root_path,
        mock_update_env,
        ...
        migration_config = Mock()
        migration_config.get_migration_product.return_value = 'SLES/15/x86_64'
        migration_config.is_zypper_migration_plugin_requested.return_value = True
    ...
    def test_is_single_rpmtrans_requested(self, mock_logger_setup, mock_open):
        ...
        mock_open.return_value.__enter__.return_value.read.return_value = 'BOOT_IMAGE=/boot/vmlinuz-5.3.18-150300.59.87-default migration.single_rpmtrans=unexpected'
        assert system.is_single_rpmtrans_requested() == '0'
